    ...
    <div class="bookmark-container">
      ...
      <div v-click-outside="outside" class="absolute bookmark-list w-1/3 xl:w-1/4 mt-4" v-if="showBookmarkPagesDropdown">
        <vx-auto-suggest
          ...
          :initalData="{Sayfalar: starredPagesLimited.concat(starredPagesMore)}"
          :searchLimit="5"
          placeholder=""
          ...
          <!-- Pages Suggestion -->
          <template v-slot:Sayfalar="{ suggestion }">
...
export default {
  ...
  computed: {
    navbarSearchAndPinList() {
      return {Sayfalar: this.$store.state.navbarSearchAndPinList["Sayfalar"]}
    },
    starredPages() {
  ...
  methods: {
    selected(obj) {
      ...
      this.$router.push(obj.Sayfalar.url).catch(() => {})
    },
    actionClicked(item) {
